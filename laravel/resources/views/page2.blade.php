    ...
    @if($posts->count())
            <div class="card my-4 text-center">
                ...
                <div class="card-body">
                    ...
                    <small>
                        <p class="mb-2">Author: <a href="/authors/{{ $posts[0]->author->username }}" class="text-decoration-none"> {{ $posts[0]->author->name }} </a> in <a href="/categories/{{ $posts[0]->category->slug }}" class="text-decoration-none"> {{ $posts[0]->category->name }}</a> | Last updated {{ $posts[0]->updated_at->diffForHumans() }}</p>
    ...
    @foreach($posts->skip(1) as $item)
            ...
            <div class="col-4 mb-3 d-flex justify-content-center">
                <div class="card" style="width: 18rem;">
                    <div class="position-absolute px-3 py-2 text-light" style="background-color: rgba(0, 0, 0, 0.7)"><a href="/categories/{{ $item->category->slug }}" class="text-decoration-none"> {{ $item->category->name }}</a></div>
